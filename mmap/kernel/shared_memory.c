...
static int __init shared_memory_init(void)
{
  ...
  shared_memory_1 = kmalloc(BUF_SIZE, GFP_KERNEL);
  if (!shared_memory_1)
  {
  ...
  memset(shared_memory_1, 0, BUF_SIZE);

  shared_memory_2 = kmalloc(BUF_SIZE, GFP_KERNEL);
  if (!shared_memory_2)
  {
  ...
  memset(shared_memory_2, 0, BUF_SIZE);

  shared_memory_3 = kmalloc(BUF_SIZE, GFP_KERNEL);
  if (!shared_memory_3)
  {
  ...
  memset(shared_memory_3, 0, BUF_SIZE);
